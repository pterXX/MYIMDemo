...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([LTSCalendarAppearance share].isShowSingleWeek) {
        ...
        self.selectDate=itemCurrent.date;
    }else{
...
- (void)setCurrentDate:(NSDate *)currentDate{
    ...
    if (self.eventSource && [self.eventSource respondsToSelector:@selector(calendarDidSelectedDate:selDate:)]) {
        [self.eventSource calendarDidSelectedDate:self.currentDate selDate:self.selectDate];
    }
}
...
- (NSArray *)getDaysOfWeek:(NSDate *)date{
    ...
    for (NSInteger i=0; i<7; i++) {
        ...
        if ([self isEqual:currentDate other:self.selectDate]) {
            item.isSelected = YES;
