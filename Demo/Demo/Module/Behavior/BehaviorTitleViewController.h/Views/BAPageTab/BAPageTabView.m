...
@interface BAPageTabView () <UIScrollViewDelegate>
//bg
...
- (void)layoutSubviews {
    if(_isNeedRefreshLayout) {
        //tab layout
        if(_tabHeight <= 0) {
            _tabHeight = kTabDefautHeight;
        }
        
        CGFloat tabWidth = WIDTH(self);
        _tabItemWidth = tabWidth/(_numberOfTabItems<_maxNumberOfPageItems?_numberOfTabItems:_maxNumberOfPageItems);
        
        self.tabView.frame = CGRectMake(0, 0, tabWidth, _tabHeight);
        self.tabView.contentSize = CGSizeMake(_tabItemWidth*_numberOfTabItems, 0);
        CGFloat border1PX =  ([[UIScreen mainScreen] scale] > 0.0 ? 1.0 / [[UIScreen mainScreen] scale] : 1.0);
        self.lineLayer.frame = CGRectMake(0, _tabHeight-border1PX, tabWidth, border1PX);
        
        for(NSInteger i = 0; i < _tabItems.count; i++) {
            BAPageTabItemLable *tabItem = (BAPageTabItemLable *)_tabItems[i];
            tabItem.frame = CGRectMake(_tabItemWidth*i, 0, _tabItemWidth, _tabHeight);
        }
        [self layoutIndicatorViewWithStyle];

        //body layout
        self.bodyView.frame = CGRectMake(0, _tabHeight, WIDTH(self), HEIGHT(self)-_tabHeight);
        ...
        for(NSInteger i = 0; i < _numberOfTabItems; i++) {
            ...
            childController.view.frame = CGRectMake(WIDTH(self)*i, 0, WIDTH(self), HEIGHT(self)-_tabHeight);
        }
    }
}
...
- (void)initBaseSettings {
    ...
    _isChangeByClick = NO;
    _bodyBounces = YES;
    _titleStyle = BAPageTabTitleStyleDefault;
    _indicatorStyle = BAPageTabIndicatorStyleDefault;
    _minScale = kMinScale;
    _selectedColorR = 1;
    _selectedColorG = 0;
    _selectedColorB = 0;
...
- (void)initTabView {
    ...
    [self.tabView.layer addSublayer:self.lineLayer];
    
    for(NSInteger i = 0; i < _numberOfTabItems; i++) {
        BAPageTabItemLable *tabItem = [[BAPageTabItemLable alloc] init];
...
- (void)reviseTabContentOffsetBySelectedIndex:(BOOL)isAnimate {
    BAPageTabItemLable *currentTabItem = _tabItems[_selectedTabIndex];
    CGFloat selectedItemCenterX = currentTabItem.center.x;
    CGFloat tabWidth = WIDTH(self);
    CGFloat reviseX;
    if(selectedItemCenterX + tabWidth/2.0 >= self.tabView.contentSize.width) {
        reviseX = self.tabView.contentSize.width - tabWidth; //不足以到中心，靠右
    } else if(selectedItemCenterX - tabWidth/2.0 <= 0) {
        reviseX = 0; //不足以到中心，靠左
    } else {
        reviseX = selectedItemCenterX - tabWidth/2.0; //修正至中心
    }
...
- (void)changeSelectedItemToNextItem:(NSInteger)nextIndex {
    BAPageTabItemLable *currentTabItem = _tabItems[_selectedTabIndex];
    BAPageTabItemLable *nextTabItem = _tabItems[nextIndex];
...
- (void)resetTabItemScale {
    for(NSInteger i = 0; i < _numberOfTabItems; i++) {
        BAPageTabItemLable *tabItem = _tabItems[i];
...
- (void)addIndicatorViewWithStyle {
    switch (_indicatorStyle) {
        case BAPageTabIndicatorStyleDefault:
        case BAPageTabIndicatorStyleFollowText:
        case BAPageTabIndicatorStyleStretch:
...
- (void)layoutIndicatorView {
    ...
    BAPageTabItemLable *selecedTabItem = _tabItems[_selectedTabIndex];
    self.indicatorView.frame = CGRectMake(selecedTabItem.center.x-indicatorWidth/2.0-_tabView.contentOffset.x, _tabHeight-_indicatorHeight, indicatorWidth, _indicatorHeight);
}
...
- (void)changeChildControllerOnClick:(UITapGestureRecognizer *)tap {
    ...
    if(nextIndex != _selectedTabIndex) {
        if(_titleStyle == BAPageTabTitleStyleDefault) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if(scrollView == self.tabView) {
        ...
        //调整title
        switch (_titleStyle) {
            case BAPageTabTitleStyleDefault:
                [self changeTitleWithDefault];
                break;
            case BAPageTabTitleStyleGradient:
                [self changeTitleWithGradient];
                break;
            case BAPageTabTitleStyleBlend:
...
- (void)changeTitleWithGradient {
    if(_leftItemIndex != _rightItemIndex) {
        ...
        BAPageTabItemLable *leftTabItem = _tabItems[_leftItemIndex];
        BAPageTabItemLable *rightTabItem = _tabItems[_rightItemIndex];
...
- (CGFloat)getIndicatorWidthWithTitle:(NSString *)title {
    if(_indicatorStyle == BAPageTabIndicatorStyleDefault || _indicatorStyle == BAPageTabIndicatorStyleStretch) {
...
- (void)setSelectedTabIndex:(NSInteger)selectedTabIndex {
    if(selectedTabIndex >= 0 && selectedTabIndex < _numberOfTabItems && _selectedTabIndex != selectedTabIndex) {
        ...
        if(_titleStyle == BAPageTabTitleStyleGradient) {
...
- (void)setSelectedColor:(UIColor *)selectedColor {
    ...
    BAPageTabItemLable *tabItem = _tabItems[_selectedTabIndex];
...
- (void)setTitleStyle:(BAPageTabTitleStyle)titleStyle {
    if(_titleStyle == BAPageTabTitleStyleDefault) {
...
- (void)setIndicatorStyle:(BAPageTabIndicatorStyle)indicatorStyle {
    if(_indicatorStyle == BAPageTabIndicatorStyleDefault) {
