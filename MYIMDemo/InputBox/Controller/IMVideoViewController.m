...
@interface IMVideoViewController ()<AVCaptureFileOutputRecordingDelegate>
...
// 视频播放
@property (nonatomic, strong) IMPlayer       *player;

@property (nonatomic, strong) IMVideoProgressView *progressView;
...
- (void)initView {
    ...
    CGFloat spacingBetweenButton = IMSCREEN_WIDTH / 4.;
    CGFloat topSpace = [UIDevice isPhoneXLater] ? 88.f : 0;
    CGFloat bottomSpace = [UIDevice isPhoneXLater] ? 83.f : 0;
    ...
    self.progressView = [IMVideoProgressView new];
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error
{
    ...
    if (self.isVideo)
    {
        ...
        if (second < 1) {
            ...
            if (!self.player) {
                self.player = [[IMPlayer alloc] initWithFrame:self.bgView.bounds withShowInView:self.bgView url:outputFileURL];
            } else {
                if (outputFileURL) {
...
{
    if (!_takeImageView) {
        CGFloat leftMargin = [UIDevice isPhoneXLater] ? 8 : 0;
        _takeImageView = [[UIImageView alloc] initWithFrame:CGRectMake(leftMargin, 0, IMSCREEN_WIDTH - leftMargin * 2, self.bgView.mj_h)];
...
//    self.afreshCenterX.constant = -(IMSCREEN_WIDTH/2/2);
//    self.ensureCenterX.constant = IMSCREEN_WIDTH/2/2;
