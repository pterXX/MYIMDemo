...
// 底部间距
#define bottomSpace ([UIDevice isPhoneXLater] ? 44 + 10 : 10)

@interface IMInputBoxView()<UITextViewDelegate> {
    BOOL isAuthorized;
}
...
- (UITextView *)inputView {
    if (!_inputView) {
        _inputView = [[UITextView alloc] initWithFrame:CGRectMake(self.voiceBtn.maxX, INPUT_BOX_BACKGROUND_SPACE, IMSCREEN_WIDTH - 3 * (INPUT_BOX_HEIGHT - 10), INPUT_BOX_HEIGHT - 2 * INPUT_BOX_BACKGROUND_SPACE)];
...
- (UIButton *)emojiBtn {
    if (!_emojiBtn) {
        _emojiBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.inputView.maxX, 0, INPUT_BOX_HEIGHT - 10, INPUT_BOX_HEIGHT)];
...
- (UIButton *)moreBtn {
    if (!_moreBtn) {
        _moreBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.emojiBtn.maxX, 0, INPUT_BOX_HEIGHT - 10, INPUT_BOX_HEIGHT)];
...
- (IMRecordView *)talkButton
{
    if (!_talkButton) {
        _talkButton = [[IMRecordView alloc] initWithFrame:CGRectMake(self.voiceBtn.maxX, INPUT_BOX_BACKGROUND_SPACE, IMSCREEN_WIDTH - 3 * (INPUT_BOX_HEIGHT - 10), INPUT_BOX_HEIGHT - 2 * INPUT_BOX_BACKGROUND_SPACE)];
...
- (UIView *)topLine {
    if (!_topLine) {
        _topLine = [[UIView alloc] initWithFrame:CGRectMake(0, 0, IMSCREEN_WIDTH, 1)];
        _topLine.backgroundColor = [IMColorTools colorWithHexString:@"0xe2e2e2"];
    }
    return _topLine;
}
...
- (void)initView {
    ...
    self.inputBoxStatus = IMInputBoxStatusNone;
...
-(void)setFrame:(CGRect)frame
{
    ...
    if (self.inputBoxStatus == IMInputBoxStatusShowVoice) {
        y = self.mj_h - INPUT_BOX_HEIGHT - IMSAFEAREA_INSETS_BOTTOM;
    }
...
- (void)clickSwitchVoiceAction:(UIButton *)sender {
    ...
    IMInputBoxStatus lastStatus = self.inputBoxStatus;
    if (self.inputBoxStatus != IMInputBoxStatusShowVoice) {
        ...
        self.inputBoxStatus = IMInputBoxStatusShowVoice;
        self.curHeight = IMTABBAR_HEIGHT;
        [self setMj_h:self.curHeight];
        
        [self.inputView resignFirstResponder];
        // 判断最后的状态是不是显示的表情
        if (lastStatus == IMInputBoxStatusShowEmoji) {
            [self.emojiBtn setImage:INPUT_BOX_FACE_IMAGE forState:UIControlStateNormal];
        }
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(inputBox:changeStatusForm:to:)]) {
            [self.delegate inputBox:self changeStatusForm:lastStatus to:IMInputBoxStatusShowVoice];
        }
    }
    else {
        
        self.inputBoxStatus = IMInputBoxStatusShowKeyboard;
        [self.inputView becomeFirstResponder];
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(inputBox:changeStatusForm:to:)]) {
            [self.delegate inputBox:self changeStatusForm:lastStatus to:IMInputBoxStatusShowKeyboard];
        }
...
- (void)clickSelecteEmoji:(UIButton *)sender {
    ...
    IMInputBoxStatus lastStatus = self.inputBoxStatus;
    if (lastStatus != IMInputBoxStatusShowEmoji) {
        self.inputBoxStatus = IMInputBoxStatusShowEmoji;
        
        if (lastStatus == IMInputBoxStatusShowKeyboard) {
            [self.inputView resignFirstResponder];
        }
        // 显示键盘图标
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(inputBox:changeStatusForm:to:)]) {
            [self.delegate inputBox:self changeStatusForm:lastStatus to:IMInputBoxStatusShowEmoji];
            // 最后的状态是显示键盘，需要先收起键盘
        }
    }
    else {
...
- (void)clickMoreAction:(UIButton *)sender {
    ...
    if (self.inputBoxStatus != IMInputBoxStatusShowMore) {
        self.inputBoxStatus = IMInputBoxStatusShowMore;
        [self.emojiBtn setImage:INPUT_BOX_FACE_IMAGE forState:UIControlStateNormal];
        [self.voiceBtn setImage:INPUT_BOX_VOICE_IMAGE forState:UIControlStateNormal];
        [self.talkButton setHidden:YES];
        [self.inputView setHidden:NO];
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(inputBox:changeStatusForm:to:)]) {
            [self.delegate inputBox:self changeStatusForm:lastStatus to:IMInputBoxStatusShowMore];
        }
    }
}
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (!isAuthorized) {
        ...
        {
            _canceled = NO;
            self.recordState = IMInputBoxRecordStatusRecording;
...
{
    if (_canceled || !isAuthorized || self.inputBoxStatus != IMInputBoxStatusShowVoice) {
        return;
    }
    CGPoint touchPoint = [[touches anyObject] locationInView:self];
    if (CGRectContainsPoint(_talkFrame, touchPoint)) {
        self.recordState = IMInputBoxRecordStatusMoveInside;
        _talkButton.titleLabel.text = @"松开 发送";
    }
    else
    {
        self.recordState = IMInputBoxRecordStatusMoveOutside;
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    CGPoint touchPoint = [[touches anyObject] locationInView:self];
    self.recordState = CGRectContainsPoint(_talkFrame, touchPoint) ? IMInputBoxRecordStatusEnd : IMInputBoxRecordStatusCancel;
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    self.recordState = IMInputBoxRecordStatusCancel;
    self.talkButton.titleLabel.text = @"按住 说话";
}
...
- (void)textViewDidBeginEditing:(UITextView *)textView {
    ...
    else if (lastStatus == IMInputBoxStatusShowMore) {
