...
@implementation IMInputBoxRecorderView
...
+ (instancetype)allocWithZone:(struct _NSZone *)zone {
    return [IMInputBoxRecorderView shareInstance];
}
...
- (void)updateState:(IMInputBoxRecordStatus)state
{
    [recorderPrompt setHidden:state == IMInputBoxRecordStatusNone];
    [self.secondLabel setHidden:state == IMInputBoxRecordStatusNone];
    
    if (state == IMInputBoxRecordStatusRecording ||
        state == IMInputBoxRecordStatusMoveInside)
    {
    ...
    else if (state == IMInputBoxRecordStatusMoveOutside)
    {
        [recorderPrompt setHidden:NO];
        ...
        recorderPrompt.prompt.backgroundColor = [IMColorTools colorWithHexString:@"0x953729"];
    }
    else if (state == IMInputBoxRecordStatusCancel ||
             state == IMInputBoxRecordStatusEnd ||
             state == IMInputBoxRecordStatusNone)
    {
        [recorderPrompt setHidden:YES];
    }
    else if (state == IMInputBoxRecordStatusTooShort)
    {
        [recorderPrompt setHidden:NO];
        [recorderPrompt.rightImageView setHidden:YES];
        [recorderPrompt.leftImageView setHidden:YES];
