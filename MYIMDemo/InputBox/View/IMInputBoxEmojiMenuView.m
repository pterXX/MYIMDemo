...
@interface IMInputBoxEmojiMenuView()

@end

@implementation IMInputBoxEmojiMenuView
...
- (void)initView {
    ...
    UIView *rightLine = [[UIView alloc] initWithFrame:CGRectMake(IMSCREEN_WIDTH - MENU_EMOJI_ITEM_HEIGHT - 20 - 0.25, 0, 0.5, MENU_EMOJI_ITEM_HEIGHT)];
    rightLine.backgroundColor = KLineColor;
    [self addSubview:rightLine];
    
    IMEmojiGroupManager *emojiManager = [IMEmojiGroupManager shareManager];
    ...
    IMEmojiGroup *lastEmojiGroup = emojiManager.currentGroup;
    ...
    if (emojiGroups.count == 1) {
        ...
        for (IMEmojiGroup *group in emojiGroups)
        {
    ...
    [emojiGroups enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        button.backgroundColor = idx == index ? [IMColorTools colorWithHexString:@"0xeeeeee"] : [UIColor whiteColor];
        button.tag = 10 + idx;
        ...
        line.backgroundColor = [IMColorTools colorWithHexString:@"0xeeeeee"];
        [weakSelf.scrollView addSubview:line];
        if (idx == index) {
            weakSelf.lastSelectEemojiGroup = button;
            button.backgroundColor = [IMColorTools colorWithHexString:@"0xeeeeee"];
        }
    }];
...
- (void)didSelectEmojiGroup:(UIButton *)sender
{
    ...
    {
        NSArray *emojiGroups     = [[IMEmojiGroupManager shareManager] emojiGroup];
        IMEmojiGroup *emojiGroup  = emojiGroups[sender.tag - 10];
        sender.backgroundColor   = [IMColorTools colorWithHexString:@"0xeeeeee"];
...
- (UIScrollView *)scrollView
{
    if (!_scrollView)
    {
        _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(44, 0, IMSCREEN_WIDTH - 2*MENU_EMOJI_ITEM_HEIGHT - 20, MENU_EMOJI_ITEM_HEIGHT)];
...
- (UIButton *)sendButton
{
    if (!_sendButton)
    {
        _sendButton = [[UIButton alloc] initWithFrame:CGRectMake(IMSCREEN_WIDTH-MENU_EMOJI_ITEM_HEIGHT - 20, 0, MENU_EMOJI_ITEM_HEIGHT+20, MENU_EMOJI_ITEM_HEIGHT)];
