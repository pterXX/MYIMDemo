//
//  IMExpressionMessageCell.m
//  IMChat
//
//  Created by 徐世杰 on 16/3/19.
...
@interface IMExpressionMessageCell ()

@property (nonatomic, strong) UIImageView *msgImageView;

@end

@implementation IMExpressionMessageCell
...
- (void)setMessage:(IMExpressionMessage *)message
{
...
//    }
    ...
    if (data) {
        ...
        if (data) {
            ...
            [self.msgImageView tt_setImageWithURL:IMURL(message.url) completed:^(UIImage *image, NSError *error, IMImageCacheType cacheType, NSURL *imageURL) {
                if ([[imageURL description] isEqualToString:[(IMExpressionMessage *)weakSelf.message url]]) {
