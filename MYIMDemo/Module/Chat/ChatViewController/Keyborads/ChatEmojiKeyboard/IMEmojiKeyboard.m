...
static IMEmojiKeyboard *emojiKB;

@implementation IMEmojiKeyboard

+ (IMEmojiKeyboard *)keyboard
{
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        emojiKB = [[IMEmojiKeyboard alloc] init];
    });
    return emojiKB;
}
...
#pragma mark - # Getter
- (IMEmojiGroupDisplayView *)displayView
{
    if (_displayView == nil) {
        _displayView = [[IMEmojiGroupDisplayView alloc] init];
...
- (IMEmojiGroupControl *)groupControl
{
    if (_groupControl == nil) {
        _groupControl = [[IMEmojiGroupControl alloc] init];
