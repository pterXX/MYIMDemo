...
@interface IMChatMailTableViewCell()
// 云邮
...
@implementation IMChatMailTableViewCell
...
- (void)initView {
    ...
    _horizontalLine.backgroundColor = [IMColorTools colorWithHexString:@"0xececec"];
    [self.messageBackgroundImageView addSubview:_horizontalLine];
    
    for (int i = 0; i < 2; i ++) {
        UIView *line = [UIView new];
        line.backgroundColor = [IMColorTools colorWithHexString:@"0xececec"];
...
- (void)layoutSubviews
{
    ...
    if (self.messageModel.direction == IMMessageSenderTypeReceiver)
    {
        ...
        [self showBottomView];
        self.messageSendStatusImageView.frame = CGRectMake(self.messageBackgroundImageView.maxX + MESSAGE_BACKGROUND_SPACE, self.messageBackgroundImageView.maxY - 20, 20, 20);
    }
    else
    {
        self.messageSendStatusImageView.hidden = self.messageModel.messageSendStatus == IMMessageSendStatusSendSuccess;
        ...
        self.messageSendStatusImageView.frame = CGRectMake(self.messageBackgroundImageView.mj_x - MESSAGE_BACKGROUND_SPACE - 20, self.messageBackgroundImageView.maxY - 20, 20, 20);
    }
    ...
    // maxY = 10 + 10 + 16
    // 收到邮件时 右对齐，发送邮件时 左对齐
    self.mailTips.textAlignment      = self.messageModel.direction == IMMessageSenderTypeReceiver ? NSTextAlignmentRight : NSTextAlignmentLeft;
...
- (void)setMessageModel:(IMMessageModel *)messageModel
{
    ...
    else
    {
        ...
        self.mailTitle.textColor             = MAIL_TITLE_TEXTCOLOR;
        self.horizontalLine.backgroundColor  = [IMColorTools colorWithHexString:@"0xececec"];
        self.firstLine.backgroundColor       = [IMColorTools colorWithHexString:@"0xececec"];
        self.secondeLine.backgroundColor     = [IMColorTools colorWithHexString:@"0xececec"];
    }
    ...
    // 设置"(数字)"颜色
    UIColor *color = messageModel.direction == IMMessageSenderTypeSender ? MAIL_DETAIL_TEXTCOLOR_SENDER : MAIL_DETAIL_TEXTCOLOR;
