...
#import "IMDBExpressionSQL.h"
#import "IMDBManager.h"
#import "IMMacros.h"

@implementation IMDBExpressionStore
...
- (BOOL)addExpressionGroup:(IMExpressionGroupModel *)group forUid:(NSString *)uid
{
    ...
    NSArray *arr = [NSArray arrayWithObjects:
                    ...
                    IMNoNilString(group.name),
                    IMNoNilString(group.groupInfo),
                    IMNoNilString(group.detail),
                    [NSNumber numberWithInteger:group.count],
                    IMNoNilString(group.authID),
                    IMNoNilString(group.authName),
                    IMTimeStamp(group.date),
                    @"", @"", @"", @"", @"", nil];
...
- (NSArray *)expressionGroupsByUid:(NSString *)uid
{
    ...
    [self excuteQuerySQL:sqlString resultBlock:^(FMResultSet *retSet) {
        while ([retSet next]) {
            IMExpressionGroupModel *group = [[IMExpressionGroupModel alloc] init];
    ...
    // 读取表情包的所有表情信息
    for (IMExpressionGroupModel *group in data) {
