...
- (void)setMessage:(IMMessage *)message
{
    ...
    if ([message.fromUser chat_avatarPath].length > 0) {
        ...
        [self.avatarButton tt_setImageWithURL:IMURL([message.fromUser chat_avatarURL]) forState:UIControlStateNormal];
    }
    ...
    if (!message || _message.ownerTyper != message.ownerTyper) {
        ...
        [self.messageBackgroundView mas_remakeConstraints:^(MASConstraintMaker *make) {
            message.ownerTyper == IMMessageOwnerTypeSelf ? make.right.mas_equalTo(self.avatarButton.mas_left).mas_offset(-MSGBG_SPACE_X) : make.left.mas_equalTo(self.avatarButton.mas_right).mas_offset(MSGBG_SPACE_X);
