...
#import "IMTalkButton.h"

@interface IMChatBar () <UITextViewDelegate>
{
    UIImage *kVoiceImage;
...
@property (nonatomic, strong) IMTalkButton *talkButton;
...
- (id)init
{
    if (self = [super init]) {
        ...
        self.status = IMChatBarStatusInit;
    }
    return self;
}
...
{
    if (self.status == IMChatBarStatusEmoji || self.status == IMChatBarStatusKeyboard || self.status == IMChatBarStatusMore) {
        return YES;
    }
    return NO;
}
...
- (BOOL)resignFirstResponder
{
    ...
    if (self.status == IMChatBarStatusKeyboard) {
        [self.textView resignFirstResponder];
        self.status = IMChatBarStatusInit;
        if (self.delegate && [self.delegate respondsToSelector:@selector(chatBar:changeStatusFrom:to:)]) {
            [self.delegate chatBar:self changeStatusFrom:self.status to:IMChatBarStatusInit];
        }
    }
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView
{
    ...
    if (self.status != IMChatBarStatusKeyboard) {
        if (_delegate && [_delegate respondsToSelector:@selector(chatBar:changeStatusFrom:to:)]) {
            [self.delegate chatBar:self changeStatusFrom:self.status to:IMChatBarStatusKeyboard];
        }
        if (self.status == IMChatBarStatusEmoji) {
            [self.emojiButton setImage:kEmojiImage imageHL:kEmojiImageHL];
        }
        else if (self.status == IMChatBarStatusMore) {
            [self.moreButton setImage:kMoreImage imageHL:kMoreImageHL];
        }
        self.status = IMChatBarStatusKeyboard;
    }
    return YES;
}
...
- (void)voiceButtonDown
{
    ...
    // 开始文字输入
    if (self.status == IMChatBarStatusVoice) {
        if (textRec.length > 0) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(chatBar:changeStatusFrom:to:)]) {
            [self.delegate chatBar:self changeStatusFrom:self.status to:IMChatBarStatusVoice];
        }
        if (self.status == IMChatBarStatusKeyboard) {
            [self.textView resignFirstResponder];
        }
        else if (self.status == IMChatBarStatusEmoji) {
            [self.emojiButton setImage:kEmojiImage imageHL:kEmojiImageHL];
        }
        else if (self.status == IMChatBarStatusMore) {
            [self.moreButton setImage:kMoreImage imageHL:kMoreImageHL];
        ...
        self.status = IMChatBarStatusVoice;
    }
}
...
- (void)emojiButtonDown
{
    ...
    if (self.status == IMChatBarStatusEmoji) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(chatBar:changeStatusFrom:to:)]) {
            [self.delegate chatBar:self changeStatusFrom:self.status to:IMChatBarStatusEmoji];
        }
        if (self.status == IMChatBarStatusVoice) {
            [self.voiceButton setImage:kVoiceImage imageHL:kVoiceImageHL];
            [self.talkButton setHidden:YES];
        ...
        [self.textView resignFirstResponder];
        self.status = IMChatBarStatusEmoji;
    }
}
...
{
    // 开始文字输入
    if (self.status == IMChatBarStatusMore) {
        if (_delegate && [_delegate respondsToSelector:@selector(chatBar:changeStatusFrom:to:)]) {
            [self.delegate chatBar:self changeStatusFrom:self.status to:IMChatBarStatusKeyboard];
        }
        [self.moreButton setImage:kMoreImage imageHL:kMoreImageHL];
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(chatBar:changeStatusFrom:to:)]) {
            [self.delegate chatBar:self changeStatusFrom:self.status to:IMChatBarStatusMore];
        }
        if (self.status == IMChatBarStatusVoice) {
            [self.voiceButton setImage:kVoiceImage imageHL:kVoiceImageHL];
            [self.talkButton setHidden:YES];
        ...
        [self.textView resignFirstResponder];
        self.status = IMChatBarStatusMore;
    }
}
...
- (IMTalkButton *)talkButton
{
    if (_talkButton == nil) {
        _talkButton = [[IMTalkButton alloc] init];
