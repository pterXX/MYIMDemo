...
@interface IMRecorderIndicatorView ()
...
@implementation IMRecorderIndicatorView
...
- (void)setStatus:(IMRecorderStatus)status
{
    if (status == IMRecorderStatusWillCancel) {
    ...
    else if (status == IMRecorderStatusRecording) {
    ...
    else if (status == IMRecorderStatusTooShort) {
        [self.centerImageView setHidden:NO];
        [self.centerImageView setImage:[UIImage imageNamed:@"chat_record_cancel"]];
        [self.titleBackgroundView setHidden:YES];
        [self.recImageView setHidden:YES];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if (status == IMRecorderStatusTooShort) {
