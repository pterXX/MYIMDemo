...
@interface IMEmojiGroupControl () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
...
@implementation IMEmojiGroupControl
...
- (id)init
{
    if (self = [super init]) {
        ...
        [self.collectionView registerClass:[IMEmojiGroupCell class] forCellWithReuseIdentifier:@"IMEmojiGroupCell"];
    }
    return self;
}

- (void)setSendButtonStatus:(IMGroupControlSendButtonStatus)sendButtonStatus
{
    if (_sendButtonStatus != sendButtonStatus) {
        if (_sendButtonStatus == IMGroupControlSendButtonStatusNone) {
            ...
            if (sendButtonStatus == IMGroupControlSendButtonStatusBlue) {
            ...
            else if (sendButtonStatus == IMGroupControlSendButtonStatusGray) {
        ...
        else if (sendButtonStatus == IMGroupControlSendButtonStatusNone) {
            [self.sendButton mas_updateConstraints:^(MASConstraintMaker *make) {
...
- (void)setCurIndexPath:(NSIndexPath *)curIndexPath
{
    if (curIndexPath.row < self.emojiGroupData.count) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(emojiGroupControl:didSelectedGroup:)]) {
            IMExpressionGroupModel *group = [self.emojiGroupData objectAtIndex:curIndexPath.row];
...
{
    IMEmojiGroupCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"IMEmojiGroupCell" forIndexPath:indexPath];
    IMExpressionGroupModel *group = [self.emojiGroupData objectAtIndex:indexPath.row];
...
{
    IMExpressionGroupModel *group = [self.emojiGroupData objectAtIndex:indexPath.row];
    if (group.type == IMEmojiTypeOther) {
