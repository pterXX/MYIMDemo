...
@interface IMChatTextTableViewCell()<UITextViewDelegate, UITextDragDelegate>
@property (nonatomic, strong) UILongPressGestureRecognizer *longPress;

@end

@implementation IMChatTextTableViewCell
...
- (IMTextView *)textView
{
    if (!_textView)
    {
        _textView                  = [[IMTextView alloc] init];
...
- (void)layoutSubviews
{
    ...
    if (self.messageModel.direction == IMMessageSenderTypeReceiver) {
        x = self.avatarImageView.maxX + MESSAGE_BACKGROUND_SPACE + MESSAGE_BACKGROUND_SPACE/2.;
    ...
    {
        self.messageSendStatusImageView.frame = CGRectMake(self.messageBackgroundImageView.mj_x - MESSAGE_BACKGROUND_SPACE/2 - 20, self.messageBackgroundImageView.maxY - 20 - 5, 20, 20);
