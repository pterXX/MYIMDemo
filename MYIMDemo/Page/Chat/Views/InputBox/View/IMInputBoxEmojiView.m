...
#import "IMEmojiCollectionViewCell.h"
#import "IMHorizontalPageFlowlayout.h"

static NSString *identifier = @"kCell";

@interface IMInputBoxEmojiView() <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDataSourcePrefetching, IMInputBoxViewDelegate>
{
    // 一行显示的表情数量
    NSInteger rowCount;
    // 一页多少行
...
// 最后选择的表情组
@property (nonatomic, strong) IMEmojiGroup *lastEmojiGroup;

@property (nonatomic, strong) UICollectionView *collectionView;

@property (nonatomic, strong) IMHorizontalPageFlowlayout *flowLayout;

@property (nonatomic, strong) IMEmojiGroupManager *emojiManager;

@end


@implementation IMInputBoxEmojiView
...
- (void)initView {
    ...
    _emojiManager = [IMEmojiGroupManager shareManager];
 ...
 */
- (void)updateDataSource:(IMEmojiGroup *)emojiGroup {
...
//    [[NSUserDefaults standardUserDefaults] setObject:emojiGroup forKey:@"lastEmojiGroup"];
    // 一行个数
    rowCount = emojiGroup.emojiType == IMEmojiTypeNomarl ? NORMARL_EMOJI_ROW_COUNT : GIF_EMOJI_ROW_COUNT;
    // 多少行
    section = emojiGroup.emojiType == IMEmojiTypeNomarl ? 3 : 2;
    // 一页显示多少表情
    pageEmoji = emojiGroup.emojiType == IMEmojiTypeNomarl ? (rowCount * section - 1) : rowCount * section;
    
    NSArray *emojiArray = _emojiManager.currentEmojiList;
    
    NSMutableArray *newDataSource = [NSMutableArray arrayWithArray:emojiArray];
    
    IMEmojiModel *delete = [IMEmojiModel new];// @{@"face_id":@"1000",@"face_name":@"[删除]", @"name":@"emoji_delete"};
    delete.emojiID = @"1000";
    delete.emojiName = @"[删除]";
    delete.name = @"emoji_delete";
    
    IMEmojiModel *empty = [IMEmojiModel new];
    empty.emojiID = @"0001";
    empty.emojiName = @"";
//    empty.name = @"";
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    IMEmojiCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
    IMEmojiModel *emojiModel = self.dataSource[indexPath.row];
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    IMEmojiCollectionViewCell *kcell = (IMEmojiCollectionViewCell *)cell;
    IMEmojiModel *emojiModel = self.dataSource[indexPath.row];
...
// 点击发送
- (void)emojiMenuView:(IMInputBoxEmojiMenuView *)menuView sendEmoji:(UIButton *)sendBut {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    NSInteger pageNum = (NSInteger)scrollView.contentOffset.x / (NSInteger)IMSCREEN_WIDTH;
    NSInteger remainder1 = (NSInteger)scrollView.contentOffset.x % (NSInteger)IMSCREEN_WIDTH;
    if (remainder1 > IMSCREEN_WIDTH/2) {
        pageNum = pageNum + 1;
    }
...
- (IMHorizontalPageFlowlayout *)flowLayout {
    if (!_flowLayout) {
        CGFloat count = NORMARL_EMOJI_ROW_COUNT;
        CGFloat columnSpace = (IMSCREEN_WIDTH - (count * ITEM_WIDTH))/(count + 1);
        _flowLayout = [[IMHorizontalPageFlowlayout alloc] initWithRowCount:3 itemCountPerRow:NORMARL_EMOJI_ROW_COUNT];
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, IMSCREEN_WIDTH, INPUT_BOX_EMOJI_HEIGHT) collectionViewLayout:self.flowLayout];
        _collectionView.backgroundColor = [IMColorTools colorWithHexString:@"0xeeeeee"];
        ...
        [_collectionView registerClass:[IMEmojiCollectionViewCell class] forCellWithReuseIdentifier:identifier];
    }
...
- (UIPageControl *)pageCtrl {
    if (!_pageCtrl) {
        _pageCtrl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.collectionView.maxY, IMSCREEN_WIDTH, INPUT_BOX_EMOJI_VIEW_HEIGHT - MENU_EMOJI_ITEM_HEIGHT - INPUT_BOX_EMOJI_HEIGHT)];
        _pageCtrl.currentPage = 0;
        _pageCtrl.pageIndicatorTintColor = [IMColorTools colorWithHexString:@"0xd8d8d8"];
        _pageCtrl.currentPageIndicatorTintColor = [IMColorTools colorWithHexString:@"0x8e8e8e"];
...
- (IMInputBoxEmojiMenuView *)menuView {
    if (!_menuView) {
        _menuView = [[IMInputBoxEmojiMenuView alloc] initWithFrame:CGRectMake(0, self.pageCtrl.maxY, IMSCREEN_WIDTH, MENU_EMOJI_ITEM_HEIGHT + IMSAFEAREA_INSETS_BOTTOM)];
