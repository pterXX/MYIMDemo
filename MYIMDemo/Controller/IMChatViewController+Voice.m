...
- (void)setPrevMessage:(IMMessageModel *)prevMessage {
    objc_setAssociatedObject(self, prevMessageKey, prevMessage, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (IMMessageModel *)prevMessage {
    return objc_getAssociatedObject(self, prevMessageKey);
}

- (void)setRecordView:(IMInputBoxRecorderView *)recordView {
    objc_setAssociatedObject(self, recordViewKey, recordView, OBJC_ASSOCIATION_RETAIN);
}

- (IMInputBoxRecorderView *)recordView {
    IMInputBoxRecorderView *recordView = objc_getAssociatedObject(self, recordViewKey);
    if (!recordView) {
        recordView = [IMInputBoxRecorderView shareInstance];
    }
    return recordView;
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    ...
    if (self.recordStatus == IMInputBoxRecordStatusRecording) {
    ...
    else if (self.recordStatus == IMInputBoxRecordStatusEnd)
    {
        ...
        if (self.recordTime < 1000) {
            ...
            [self.recordView updateState:IMInputBoxRecordStatusTooShort];
            [self removeVoiceMessage];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                if (self.recordStatus == IMInputBoxRecordStatusTooShort) {
    ...
    else if (self.recordStatus == IMInputBoxRecordStatusCancel) {
        // 停止录音
        [self stopRecord];
        self.recordView.hidden = YES;
        [self removeVoiceMessage];
 ...
 */
- (void)initVoiceData {
    self.recordView = [IMInputBoxRecorderView shareInstance];
 ...
 */
- (void)playAudioWithTableViewCell:(id)tableViewCell messageModel:(IMMessageModel *)messageModel {
    
    IMChatVoiceTableViewCell *cell = tableViewCell;
    ...
    if (messageModel.fileData) {
        ...
        if ([voicePath containsString:@"/storage/msgs/"]) {
            ...
            kWeakSelf;
            [[IMFileCache shareInstance] fileUrl:messageModel.content type:@"aac" saveComplete:^(NSString *filePath, NSURL *url, NSError *error) {
                dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)recordStartDrawView {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        IMMessageModel *audioMessage    = [[IMMessageModel alloc] init];
        audioMessage.msgType           = IMMessageTypeVoice;
        audioMessage.messageChatType   = IMMessageChatTypeSingle;
        audioMessage.direction         = IMMessageSenderTypeSender;
        audioMessage.messageReadStatus = IMMessageReadStatusRead;
        audioMessage.messageSendStatus = IMMessageSendStatusSendSuccess;
    ...
    if (!recordFinished)
    {
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                IMChatVoiceTableViewCell *cell = [self.listView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:self.recordIndexPathRow inSection:0]];
            ...
            IMMessageModel *audioMessage    = weakSelf.dataSource[weakSelf.recordIndexPathRow];
            audioMessage.messageSendStatus = IMMessageSendStatusSending;
...
- (void)removeVoiceMessage {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.recordIndexPathRow < self.dataSource.count) {
            IMMessageModel *messageModel = self.dataSource[self.recordIndexPathRow];
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                if (self.recordIndexPathRow < self.dataSource.count) {
                    ...
                    for (NSInteger i = self.dataSource.count - 1; i > maxCount; i --) {
                        IMMessageModel *messageModel = self.dataSource[i];
                        if (messageModel.msgType == IMMessageTypeVoice) {
