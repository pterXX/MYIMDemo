...
#import "IMDBMessageStoreSQL.h"
#import "IMMacros.h"

@implementation IMDBMessageStore
...
- (BOOL)addMessage:(IMMessage *)message
{
    ...
    NSString *fid = @"";
    NSString *subfid;
    if (message.partnerType == IMPartnerTypeUser) {
...
- (void)messagesByUserID:(NSString *)userID partnerID:(NSString *)partnerID fromDate:(NSDate *)date count:(NSUInteger)count complete:(void (^)(NSArray *, BOOL))complete
{
    ...
    [self excuteQuerySQL:sqlString resultBlock:^(FMResultSet *retSet) {
        while ([retSet next]) {
            IMMessage *message = [self p_createDBMessageByFMResultSet:retSet];
...
- (IMMessage *)p_createDBMessageByFMResultSet:(FMResultSet *)retSet
{
    IMMessageType type = [retSet intForColumn:@"msg_type"];
    IMMessage * message = [IMMessage createMessageByType:type];
