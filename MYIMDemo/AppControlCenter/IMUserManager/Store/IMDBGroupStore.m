...
#import "IMDBGroupSQL.h"
#import "IMGroup.h"

@implementation IMDBGroupStore
...
- (BOOL)updateGroupsData:(NSArray *)groupData forUid:(NSString *)uid
{
    ...
    if (oldData.count > 0) {
        ...
        for (IMGroup *group in groupData) {
            [newDataHash setValue:@"YES" forKey:group.groupID];
        }
        for (IMGroup *group in oldData) {
...
- (NSMutableArray *)groupsDataByUid:(NSString *)uid
{
    ...
    [self excuteQuerySQL:sqlString resultBlock:^(FMResultSet *retSet) {
        while ([retSet next]) {
            IMGroup *group = [[IMGroup alloc] init];
...
- (BOOL)addGroupMember:(IMUser *)user forUid:(NSString *)uid andGid:(NSString *)gid
{
