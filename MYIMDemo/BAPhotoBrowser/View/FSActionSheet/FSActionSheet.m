...
- (CGFloat)contentHeight {
    ...
    CGFloat contentHeight = titleHeight + rowHeightSum + IMSAFEAREA_INSETS_BOTTOM;
...
- (void)showWithSelectedCompletion:(FSActionSheetHandler)selectedHandler {
    ...
    // 内容高度
    CGFloat contentHeight = [self contentHeight];//self.tableView.contentSize.height;
    // 适配屏幕高度
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    return (section == 1) ? kFSActionSheetSectionHeight : 0;
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, IMSCREEN_WIDTH, kFSActionSheetSectionHeight)];
