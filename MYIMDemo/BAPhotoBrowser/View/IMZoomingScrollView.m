...
@interface IMZoomingScrollView () <UIScrollViewDelegate, IMVideoPlayViewDelegate>
{
...
- (IMVideoPlayView *)videoPlayer {
    if (!_videoPlayer) {
        _videoPlayer = [[IMVideoPlayView alloc] init];
...
- (UILabel *)stateLabel
{
    if (_stateLabel == nil) {
        ...
        _stateLabel.text = IMPhotoBrowserLoadNetworkImageFail;
...
- (void)setShowHighQualityImageWithPhoto:(IMPhoto *)photo
{
    ...
    self.progressView.mode = IMProgressViewProgressMode;
    self.imageURL = url;
    ...
    if ([photo.imageUrl containsString:@"/storage/msgs"]) {
        ...
        [weakSelf.photoImageView sd_setImageWithURL:url placeholderImage:photo.defaultImage options:SDWebImageRetryFailed | SDWebImageLowPriority | SDWebImageHandleCookies progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            ...
            if (error) {
                ...
                IMPBLog(@"加载图片失败 , 图片链接imageURL = %@ , 错误信息: %@ ,检查是否开启允许HTTP请求",imageURL,error);
            } else {
...
- (void)setMaxAndMinZoomScales
{
    if (!_photo.videoUrl)
    {
        ...
        if (self.photoImageView.mj_h >IMSCREEN_HEIGHT) {
            self.photoImageView.mj_y = 0;
            self.scrollview.scrollEnabled = YES;
        } else {
            self.photoImageView.mj_y = (IMSCREEN_HEIGHT - self.photoImageView.mj_h ) * 0.5;
            self.scrollview.scrollEnabled = NO;
        }
        self.scrollview.maximumZoomScale = MAX(IMSCREEN_HEIGHT / self.photoImageView.mj_h, 3.0);
        self.scrollview.minimumZoomScale = 1.0;
        self.scrollview.zoomScale = 1.0;
        self.scrollview.contentSize = CGSizeMake(self.photoImageView.mj_w, MAX(self.photoImageView.mj_h, IMSCREEN_HEIGHT));
    }
    else
    {
        self.photoImageView.frame = CGRectMake(0, 0, IMSCREEN_WIDTH, IMSCREEN_HEIGHT);
