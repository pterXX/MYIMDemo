...
#import "IMVideoPlayView.h"
#import "IMZoomingScrollView.h"
#import "FSActionSheetConfig.h"
#import "IMPhotoBrowserConfig.h"

#define BaseTag 100

@interface IMPhotoBrowser () <IMZoomingScrollViewDelegate , UIScrollViewDelegate>
...
@property(nonatomic, strong) NSArray<IMPhoto *> *images;
@property (nonatomic, strong) IMZoomingScrollView *lastZoomingScrollView;

@end

@implementation IMPhotoBrowser
...
- (IMPhoto *)placeholderImage
{
    if (!_placeholderImage) {
        _placeholderImage = [[IMPhoto alloc] init];
    }
...
- (UIWindow *)photoBrowserWindow
{
    if (!_photoBrowserWindow) {
        ...
        tempVC.view.backgroundColor = IMPhotoBrowserBackgrounColor;
        ;
...
- (void)setPageControlStyle:(IMPhotoBrowserPageControlStyle)pageControlStyle
{
    // KLogFunc;
...
- (void)setBrowserStyle:(IMPhotoBrowserStyle)browserStyle
{
    _browserStyle = browserStyle;
    [self updateIndexVisible];
}

- (void)setPageControlAliment:(IMPhotoBrowserPageControlAliment)pageControlAliment
{
    _pageControlAliment = pageControlAliment;
    switch (self.pageControlAliment) {
        case IMPhotoBrowserPageControlAlimentLeft:
        {
            self.pageControl.mj_x = 10;
        }
            break;
        case IMPhotoBrowserPageControlAlimentRight:
        {
            self.pageControl.mj_x = (self.mj_w - self.pageControl.mj_w) - 10;
        }
            break;
        case IMPhotoBrowserPageControlAlimentCenter:
        default:
        {
...
- (void)initial
{
    self.backgroundColor = IMPhotoBrowserBackgrounColor;
    self.visibleZoomingScrollViews = [[NSMutableSet alloc] init];
    self.reusableZoomingScrollViews = [[NSMutableSet alloc] init];
    [self placeholderImage];
    
    _pageControlAliment = IMPhotoBrowserPageControlAlimentCenter;
    _pageControlDotSize = CGSizeMake(10, 10);
    _pageControlStyle = IMPhotoBrowserPageControlStyleAnimated;
    _hidesForSinglePage = YES;
    _currentPageDotColor = [UIColor whiteColor];
    _pageDotColor = [UIColor lightGrayColor];
    _browserStyle = IMPhotoBrowserStylePageControl;
...
- (void)iniaialUI
{
    ...
    // 添加IMPhotoBrowserStyleSimple相关控件
...
- (void)setUpPageControl
{
    ...
    switch (self.pageControlStyle) {
        case IMPhotoBrowserPageControlStyleAnimated:
        {
            ...
            break;
        case IMPhotoBrowserPageControlStyleClassic:
        {
...
- (void)orientationDidChange
{
    ...
    IMZoomingScrollView *temp = [self zoomingScrollViewAtIndex:self.currentImageIndex];
...
- (void)updateFrames
{
    ...
    rect.size.width += IMPhotoBrowserImageViewMargin;
...
{
    IMZoomingScrollView *currentZoomingScrollView = [self zoomingScrollViewAtIndex:self.currentImageIndex];
    if (longPress.state == UIGestureRecognizerStateBegan) {
        IMPBLog(@"UIGestureRecognizerStateBegan , currentZoomingScrollView.progress %f",currentZoomingScrollView.progress);
...
- (void)saveImage
{
    IMZoomingScrollView *zoomingScrollView = [self zoomingScrollViewAtIndex:self.currentImageIndex];
    if (zoomingScrollView.progress < 1.0) {
        self.savaImageTipLabel.text = IMPhotoBrowserLoadingImageText;
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    ...
    [self addSubview:self.savaImageTipLabel];
    if (error) {
        self.savaImageTipLabel.text = IMPhotoBrowserSaveImageFailText;
    } else {
        self.savaImageTipLabel.text = IMPhotoBrowserSaveImageSuccessText;
    }
    [self.savaImageTipLabel performSelector:@selector(removeFromSuperview) withObject:nil afterDelay:1.0];
}
...
- (void)showPhotos
{
    ...
    for (IMZoomingScrollView *zoomingScrollView in self.visibleZoomingScrollViews) {
...
{
    for (IMZoomingScrollView* view in self.visibleZoomingScrollViews) {
...
/**
 *  获取指定位置的IMZoomingScrollView , 三级查找,正在显示的池,回收池,创建新的并赋值
 *
 *  @param index 指定位置索引
 */
- (IMZoomingScrollView *)zoomingScrollViewAtIndex:(NSInteger)index
{
    for (IMZoomingScrollView* zoomingScrollView in self.visibleZoomingScrollViews) {
    ...
    IMZoomingScrollView* zoomingScrollView = [self dequeueReusableZoomingScrollView];
    [self setUpImageForZoomingScrollViewAtIndex:index];
    return zoomingScrollView;
}
...
/**
 *  从缓存池中获取一个IMZoomingScrollView对象
 */
- (IMZoomingScrollView *)dequeueReusableZoomingScrollView
{
    IMZoomingScrollView *photoView = [self.reusableZoomingScrollViews anyObject];
    if (photoView) {
        [self.reusableZoomingScrollViews removeObject:photoView];
    } else {
        photoView = [[IMZoomingScrollView alloc] init];
    }
    return photoView;
}
 ...
 */
- (IMPhoto *)placeholderImageForIndex:(NSInteger)index
{
    if (self.datasource && [self.datasource respondsToSelector:@selector(photoBrowser:placeholderImageForIndex:)]) {
        IMPhoto *photo = [self.datasource photoBrowser:self placeholderImageForIndex:index];
//        return [self.datasource photoBrowser:self placeholderImageForIndex:index];
        ...
        if ([self.images[index] isKindOfClass:[IMPhoto class]]) {
 ...
 */
- (IMPhoto *)highQualityImageURLForIndex:(NSInteger)index
{
    if (self.datasource && [self.datasource respondsToSelector:@selector(photoBrowser:placeholderImageForIndex:)]) {
//        NSURL *url = [self.datasource photoBrowser:self highQualityImageURLForIndex:index];
        IMPhoto *photo = [self.datasource photoBrowser:self placeholderImageForIndex:index];
        if ([photo isEmpty]) {
            IMPBLog(@"你所设置的需要显示的数据为空:%zd",index);
...
- (void)showFirstImage
{
    ...
    if (!self.sourceImageView) {
        if(self.datasource && [self.datasource respondsToSelector:@selector(photoBrowser:sourceImageViewForIndex:)]) {
            ...
            IMPBLog(@"需要提供源视图才能做弹出/退出图片浏览器的缩放动画");
            return;
        }
    }
    startRect = [self.sourceImageView.superview convertRect:self.sourceImageView.frame toView:self];
    
    IMPhoto *photo = [self placeholderImageForIndex:self.currentImageIndex];
    ...
    CGFloat width = IMSCREEN_WIDTH;
    CGFloat height = IMSCREEN_WIDTH / imageWidthHeightRatio;
    CGFloat x = 0;
    CGFloat y;
    if (height > IMSCREEN_HEIGHT) {
        y = 0;
    } else {
        y = (IMSCREEN_HEIGHT - height ) * 0.5;
    }
    ...
    [UIView animateWithDuration:IMPhotoBrowserShowImageAnimationDuration animations:^{
...
#pragma mark - IMZoomingScrollViewDelegate
/**
 *  单击图片,退出浏览
 */
- (void)zoomingScrollView:(IMZoomingScrollView *)zoomingScrollView singleTapDetected:(UITapGestureRecognizer *)singleTap
{
    ...
    [UIView animateWithDuration:IMPhotoBrowserHideImageAnimationDuration animations:^{
...
- (void)updateIndexVisible
{
    switch (self.browserStyle) {
        case IMPhotoBrowserStylePageControl:
        {
            ...
            break;
        case IMPhotoBrowserStyleIndeKabel:
        {
            ...
            break;
        case IMPhotoBrowserStyleSimple:
        {
            self.indeKabel.hidden = NO;
 ...
 *
 */
+ (instancetype)showPhotoBrowserWithCurrentImageIndex:(NSInteger)currentImageIndex imageCount:(NSUInteger)imageCount datasource:(id<IMPhotoBrowserDatasource>)datasource
{
    IMPhotoBrowser *browser = [[IMPhotoBrowser alloc] init];
 ...
 *  @param delegate               IMPhotoBrowserDelegate
 *  @param cancelButtonTitle      取消按钮文字
 *  @param deleteButtonTitle      删除按钮文字
 *  @param otherButtonTitle       其他按钮数组
 */
- (void)setActionSheetWithTitle:(nullable NSString *)title delegate:(nullable id<IMPhotoBrowserDelegate>)delegate cancelButtonTitle:(nullable NSString *)cancelButtonTitle deleteButtonTitle:(nullable NSString *)deleteButtonTitle otherButtonTitles:(nullable NSString *)otherButtonTitle, ...
{
...
#pragma mark - public method -> IMPhotoBrowser简易使用方式:一行代码展示
...
+ (instancetype)showPhotoBrowserWithImages:(NSArray *)images currentImageIndex:(NSInteger)currentImageIndex
{
    if (images.count <=0 || images ==nil) {
        IMPBLog(@"一行代码展示图片浏览的方法,传入的数据源为空,请检查传入数据源");
        return nil;
    }
    
    //检查数据源对象是否非法
    for (id image in images) {
        if (![image isKindOfClass:[IMPhoto class]]) {
            IMPBLog(@"识别到非法数据格式,请检查传入数据是否为 NSString/NSURL/ALAsset 中一种");
            return nil;
        }
    }
