//
//  IMFlexibleLayoutViewController+OldAPI.m
//  zhuanzhuan
//
...
@implementation IMFlexibleLayoutViewController (OldAPI)
...
- (BOOL)setSectionHeaderViewWithModel:(id)model forSection:(NSInteger)sectionTag className:(NSString *)className
{
    ...
    IMFlexibleLayoutSectionModel *sectionModel = [self sectionModelForTag:sectionTag];
    if (sectionModel) {
        IMFlexibleLayoutViewModel *viewModel = [[IMFlexibleLayoutViewModel alloc] initWithClassName:className andDataModel:model];
        sectionModel.headerViewModel = viewModel;
        return YES;
    }
    IMFLEXLog(@"!!!!! section不存在: %ld", (long)sectionTag);
    return NO;
}
...
- (NSArray<NSIndexPath *> *)p_addCellsWithModelArray:(NSArray *)modelArray forSection:(IMFlexibleLayoutSectionModel *)sectionModel className:(NSString *)className tag:(NSInteger)tag
{
    ...
    for (id model in modelArray) {
        IMFlexibleLayoutViewModel *viewModel = [[IMFlexibleLayoutViewModel alloc] initWithClassName:className andDataModel:model viewTag:tag];
...
- (NSArray<NSIndexPath *> *)p_insertCellsWithModelArray:(NSArray *)modelArray forSection:(IMFlexibleLayoutSectionModel *)sectionModel className:(NSString *)className tag:(NSInteger)tag pos:(NSInteger)pos
{
    RegisterCollectionViewCell(self.collectionView, className);
    if (modelArray.count == 0 || !sectionModel) {
        return nil;
...
- (BOOL)deleteCellsAtIndexPaths:(NSArray<NSIndexPath *> *)indexPaths
{
    if (indexPaths.count > 0) {
        ...
        for (IMFlexibleLayoutSectionModel *sectionModel in self.data) {
            ...
            for (IMFlexibleLayoutViewModel *viewModel in items) {
...
- (BOOL)p_deleteCellForSection:(IMFlexibleLayoutSectionModel *)sectionModel model:(id)model deleteAll:(BOOL)all
{
    BOOL ok = NO;
    if (sectionModel) {
        NSArray *data = sectionModel.itemsArray.mutableCopy;
        for (IMFlexibleLayoutViewModel *viewMdoel in data) {
...
- (BOOL)p_deleteCellForSection:(IMFlexibleLayoutSectionModel *)sectionModel className:(NSString *)className
{
    BOOL ok = NO;
    if (sectionModel) {
        NSArray *data = sectionModel.itemsArray.mutableCopy;
...
{
    NSArray<IMFlexibleLayoutViewModel *> *viewModels = [self viewModelsAtIndexPaths:indexPaths];
    for (IMFlexibleLayoutViewModel *viewModel in viewModels) {
        [viewModel updateViewHeight];
    }
}
...
- (NSArray<NSIndexPath *> *)cellIndexPathForCellTag:(NSInteger)cellTag
{
    ...
    for (int section = 0; section < self.data.count; section++) {
        ...
        for (int row = 0; row < sectionModel.itemsArray.count; row++) {
            IMFlexibleLayoutViewModel *viewModel = [sectionModel objectAtIndex:row];
...
- (NSArray<NSIndexPath *> *)cellIndexPathForSectionTag:(NSInteger)sectionTag cellTag:(NSInteger)cellTag
{
    ...
    IMFlexibleLayoutSectionModel *sectionModel = self.data[sectionIndex];
    for (int row = 0; row < sectionModel.itemsArray.count; row++) {
        IMFlexibleLayoutViewModel *viewModel = [sectionModel objectAtIndex:row];
        if (viewModel.viewTag == cellTag) {
...
#pragma mark - ## IMFlexibleLayoutViewController (OldSeperatorAPI)
@implementation IMFlexibleLayoutViewController (OldSeperatorAPI)
...
- (NSIndexPath *)p_addSeperatorCellForSection:(IMFlexibleLayoutSectionModel *)section withSize:(CGSize)size andColor:(UIColor *)color
{
    IMFlexibleLayoutSeperatorModel *model = [[IMFlexibleLayoutSeperatorModel alloc] initWithSize:size andColor:color];
